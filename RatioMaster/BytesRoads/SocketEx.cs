...
namespace RatioMaster.BytesRoads {
  ...
  public class SocketEx : IDisposable {
    ...
    private OpState _opState = OpState.Finished;
    private bool _disposed;
    private Timer _timer;

    private int _recvTimeout = Timeout.Infinite;
    private int _sendTimeout = Timeout.Infinite;
    private int _acceptTimeout = Timeout.Infinite;
    private int _connectTimeout = Timeout.Infinite;

    /// <summary>
    /// Initializes  new instance of the 
    /// <see cref="SocketEx"/> class for 
    /// direct connection.
    /// </summary>
    /// <remarks>
    /// With this constructor the instance of the 
    /// <see cref="SocketEx"/> class would
    ...
    /// <summary>
    /// Initializes  new instance of the 
    /// <see cref="BytesRoads.SocketEx"/> class for 
    ...
    /// One of the <see cref="BytesRoads.ProxyType"/> values.
    /// </param>
    ...
    /// <see cref="BytesRoads.SocketEx.Connect"/>
    /// method will fail.
    /// </remarks>
    public SocketEx(
      ProxyType proxyType,
      ...
      if (ProxyType.None == proxyType)
        ...
        var msg = string.Format("Proxy type is not supported ({0}).", proxyType.ToString());
...
*/
    ...
    /// The <see cref="SocketEx"/> object was disposed.
    /// </exception>
    ...
    /// in constructor of the <see cref="SocketEx"/> class.
    /// </remarks>
    ...
    /// <see cref="BytesRoads.SocketEx.Send"/> and
    /// <see cref="BytesRoads.SocketEx.BeginSend"/>
    /// operation.
    /// </summary>
    /// <value>
    ...
    /// </value>
    /// <exception cref="System.ObjectDisposedException">
    /// The <see cref="BytesRoads.SocketEx"/> object was disposed.
    /// </exception>
    ...
    /// <summary>
    /// Gets or sets timeout value for the 
    /// <see cref="BytesRoads.SocketEx.Receive"/> and
    /// <see cref="BytesRoads.SocketEx.BeginReceive"/>
    /// operation.
    /// </summary>
    /// <value>
    /// Integer value which indicate timeout in milliseconds.
    ...
    /// <see cref="BytesRoads.SocketEx.Connect">Connect</see>
    /// and
    /// <see cref="BytesRoads.SocketEx.BeginConnect">BeginConnect</see>
    /// methods.
    /// </remarks>
    /// <exception cref="System.ObjectDisposedException">
    /// The <see cref="BytesRoads.SocketEx"/> object was disposed.
    ...
    // private Exception NonCLSException => new Exception("Non CLS-compliant exception was thrown.");
    ...
    private int GetTimeoutValue(int val, string propName) {
      if (val < 0 && Timeout.Infinite != val)
        throw new ArgumentOutOfRangeException(propName, val,
          "Timeout value should not be less then zero (exception is only Timeout.Infinite");
    ...
    private object DoTimeoutOp(int timeout, IOp op) {
      ...
      try {
        ...
        NsTrace.WriteLineError("SocketEx (ex): " + e);
        StopTimeoutTrack(e);
        throw;
      }
    ...
    private object BeginTimeoutOp(int timeout, IOp op, AsyncCallback cb, object state) {
      ...
      try {
        ...
        NsTrace.WriteLineError("SocketEx.B (ex): " + e);
        StopTimeoutTrack(e);
        throw;
      }
    ...
    private object EndTimeoutOp(IOp op, IAsyncResult ar) {
      ...
      try {
        ...
        NsTrace.WriteLineError("SocketEx.E (ex): " + e);
        StopTimeoutTrack(e);
        throw;
      }
    ...
    private class AcceptOp : IOp {
      private SocketBase _baseSocket;

      internal AcceptOp(SocketBase baseSocket) {
        _baseSocket = baseSocket;
      }
    ...
    /// <summary>
    /// Creates the new instance of the <see cref="SocketEx"/>
    /// class which represents newly created connection.
    /// </summary>
    /// <returns>
    /// An instance of the <see cref="SocketEx"/>
    ...
    /// (<see cref="BytesRoads.ProxyType.HttpConnect"/> proxy type) 
    /// will throw the 
    ...
    public SocketEx Accept() {
      return (SocketEx) DoTimeoutOp(_acceptTimeout, new AcceptOp(_baseSocket));
    }
    ...
    public SocketEx EndAccept(IAsyncResult asyncResult) {
      return (SocketEx) EndTimeoutOp(new AcceptOp(_baseSocket), asyncResult);
    }
    ...
    /// started in the 
    /// <see cref="BytesRoads.SocketEx.BeginConnect"/> method.
    /// </remarks>
    /// 
    /// <exception cref="System.ObjectDisposedException">
    /// The <see cref="BytesRoads.SocketEx"/> object was disposed.
    ...
    private class Bind_Op : IOp {
      private SocketBase _baseSocket;
      private SocketEx _primConnSock; // primary connection (used for socks proxy)
    ...
    /// <summary>
    /// Associates a <see cref="SocketEx"/> with
    ...
    /// <item>
    /// <term>
    /// <see cref="BytesRoads.ProxyType.None"/>
    /// </term>
    /// <description>
    ...
    /// <item>
    /// <term>
    /// <see cref="BytesRoads.ProxyType.HttpConnect"/>
    /// </term>
    /// <description>
    ...
    /// <item>
    /// <term>
    /// <see cref="BytesRoads.ProxyType.Socks4"/>
    /// <para>-or-</para>
    /// <see cref="BytesRoads.ProxyType.Socks4a"/>
    /// <para>-or-</para>
    /// <see cref="BytesRoads.ProxyType.Socks5"/>
    /// </term>
    /// <description>
    /// The socket will be bound at the proxy server.
    /// Port and IP address may be retrieved via 
    /// <see cref="LocalEndPoint"/> property.
    ...
    /// <see cref="SocketEx"/> class which configured
    ...
    /// <see cref="SocketEx"/> 
    /// connection with the <see cref="Shutdown"/> 
    ...
    /// </summary>
    /// <remarks>
    /// Internally simply calls <see cref="Dispose"/>.
    /// </remarks>
    public void Close() {
      Dispose();
    }
    ...
    /// <see cref="Dispose">Dispose()</see>
    /// method and the 
    /// <see cref="Finalize">Finalize</see>
